...
const ConverterSection: React.FC<ConverterSectionProps> = ({
  ...
  const [converterBaseUrl, setConverterBaseUrl] = useState(
    'https://api.dhl.com/ecs/ppl/myapi2' // Výchozí URL
  );
  ...
  const handleTransform = useCallback(() => {
    if (!soapInput.trim()) {
      setRestOutput({
        success: false,
        error: t('enterSoapXml') as string,
      });
      return;
    }
    ...
    try {
      ...
      let transformationResultPart: TransformationResultPart | null = null;

      // Mapa názvů SOAP operací na jejich příslušné transformační funkce
      ...
      for (const opKey in operationTransformers) {
        if (new RegExp(`<\\s*(\\w+:)?${opKey}[^>]*>`, 'i').test(soapInput)) {
          operationName = opKey;
          const transformer = operationTransformers[opKey];
          if (transformer) {
            transformationResultPart = transformer(soapInput, t);
          } else {
            // Tento případ by neměl nastat, pokud máme všechny operace v mapě
            console.warn(
              `Transformační funkce pro ${opKey} není definována v operationTransformers.`
            );
            setRestOutput({
              success: false,
              error: `${
                t('converterUnimplementedOperation') as string
              } (${opKey})`,
            });
            return;
          }
      ...
      if (operationName && transformationResultPart) {
        setRestOutput({
          success: true, 
          operation: operationName,
          method: transformationResultPart.method,
          path: transformationResultPart.path,
          body: transformationResultPart.body,
          queryParams: transformationResultPart.queryParams,
          notes: transformationResultPart.notes,
        });
      } else if (!operationName) {
        // Pokud žádná známá operace nebyla v XML detekována
        setRestOutput({
          success: false,
  ...
  return (
    <div className="mt-2">
      ...
      {/* Formulář pro vstup a zobrazení výstupu */}
